...
def update_ai_config(recommendations):
    ...
    if 'hand_piece_bonus' in recommendations:
        old_value = config['evaluation']['hand_piece_bonus_multiplier']
        new_value = recommendations['hand_piece_bonus']
...
if __name__ == "__main__":
    ...
    # After analysis, check if we should update config
    if drop_diff > 0.10:  # Significant drop rate difference
        recommendations = {
            'hand_piece_bonus': 1.3  # Increase from current value
        }
        print("\n⚠️  Significant drop rate difference detected!")
        print("Automatically updating ai_config.json...")
        update_ai_config(recommendations)
